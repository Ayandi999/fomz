...
export function VideoModal({ videoOpen, setVideoOpen }: VideoModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/85 backdrop-blur-md p-6">
      <div className="relative w-full max-w-4xl bg-white border border-black/10 rounded-none overflow-hidden shadow-2xl flex flex-col">
        <div className="flex items-center justify-between p-4 bg-white/60 border-b border-black/5">
          <span className="text-xs font-black uppercase tracking-widest text-[#2563EB]">Formz Product Tour</span>
          <button 
        ...
        <div className="aspect-video w-full bg-[#fdfbf7] flex flex-col items-center justify-center p-6 text-center">
          <div className="max-w-md flex flex-col items-center gap-4">
            <div className="w-16 h-16 rounded-full bg-[#2563EB]/10 flex items-center justify-center border border-[#2563EB]/20">
            ...
            <button
              onClick={() => setVideoOpen(false)}
              className="mt-2 bg-[#2563EB] text-white font-bold px-6 py-2.5 rounded-full hover:bg-[#2563EB]/90 transition-all text-xs uppercase tracking-widest"
            >
              Close Player
            </button>
          </div>
