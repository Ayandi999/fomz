    ...
    <div className="min-h-screen w-full flex flex-col bg-[#0F0F0F] text-foreground antialiased font-sans pb-16">
      ...
      <header className="w-full border-b border-border bg-[#0F0F0F] sticky top-0 z-50">
        <div className="w-full max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            ...
            <Link 
              href="/sign-up"
              className="bg-[#2563EB] hover:bg-[#2563EB]/90 text-white font-bold px-4 py-2 rounded-none text-xs uppercase tracking-widest transition-colors shadow-sm"
            >
              Get Started Free
            </Link>
          </div>
      ...
      <main className="w-full max-w-7xl mx-auto px-6 py-12 flex flex-col gap-10">
        ...
        <div className="flex flex-col gap-3 max-w-2xl">
          <span className="text-xs uppercase tracking-widest text-[#2563EB] font-black flex items-center gap-1.5">
            <Sparkles className="w-3.5 h-3.5 text-[#2563EB] animate-pulse" /> Community Flows
          </span>
        ...
        <div className="flex items-center gap-3">
          <div className="relative w-full max-w-md bg-[#161616] border border-border rounded-none p-1 flex items-center shadow-lg focus-within:border-primary/50 transition-colors">
          ...
          <button
            ...
            className="flex items-center justify-center p-3 rounded-none bg-[#161616] border border-border hover:border-primary/50 text-[#666] hover:text-white transition-all shadow-lg group disabled:opacity-50"
            title="Refresh Explore Feed"
          >
            <RefreshCw className={`w-5 h-5 ${(isRefreshing || isLoading) ? "animate-spin text-[#2563EB]" : "group-hover:text-[#2563EB]"}`} />
          </button>
        </div>
        ...
        {isError && (
          <div className="flex items-center gap-3 border border-red-500/20 bg-red-500/5 p-4 rounded-none max-w-lg">
        ...
        {!isLoading && !isError && (
          <>
            {filteredForms.length === 0 ? (
              <div className="border border-dashed border-border rounded-none py-24 px-6 flex flex-col items-center justify-center gap-3 text-center bg-card/20 max-w-lg">
                ...
                {filteredForms.map((form) => {
                  ...
                  return (
                    <a
                      ...
                      className="group bg-[#161616] hover:bg-[#1A1A1A] border border-border/40 hover:border-primary/40 p-6 rounded-none flex flex-col justify-between gap-6 transition-all duration-300 cursor-pointer shadow-[0_4px_24px_rgba(0,0,0,0.3)] hover:shadow-[0_8px_32px_rgba(0,0,0,0.4)] hover:scale-[1.01] no-underline"
                    >
                      <div className="flex flex-col gap-4">
                        
                        {/* Header Details */}
                        <div className="flex items-start justify-between gap-3">
                          <div className="w-10 h-10 rounded-none bg-[#0F0F0F] flex items-center justify-center shrink-0 border border-border/10">
