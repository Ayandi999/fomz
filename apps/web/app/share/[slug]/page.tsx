    ...
    return (
      <main className="min-h-screen flex flex-col items-center justify-center bg-neutral-950 text-white p-6">
        <div className="flex flex-col items-center gap-6 text-center max-w-lg animate-fade-in">
          <div className="w-20 h-20 rounded-full bg-primary/20 border-2 border-orange-400 flex items-center justify-center">
          ...
          {currentField.fieldType === "WELCOME" && (
            <div className="flex flex-col gap-4 items-start">
              ...
              <button
                type="button"
                onClick={handleNext}
                className="mt-4 flex items-center gap-2 bg-primary text-black font-black uppercase tracking-widest text-sm px-6 py-3 hover:bg-primary/80 transition-colors"
              >
          ...
          {currentField.fieldType === "INFO" && (
            <div className="flex flex-col gap-4">
              ...
              <button
                type="button"
                onClick={handleNext}
                className="mt-4 flex items-center gap-2 border-2 border-neutral-600 text-white font-black uppercase tracking-widest text-sm px-6 py-3 hover:border-orange-400 hover:text-orange-400 transition-colors w-fit"
              >
          ...
          {!["WELCOME", "INFO", "THANK_YOU"].includes(currentField.fieldType) && (
            <>
              ...
              {currentField.fieldType === "EMAIL" && (
                <div className="flex items-center gap-3 border-b-2 border-neutral-700 focus-within:border-primary transition-colors py-3">
              ...
              {currentField.fieldType === "WEBSITE" && (
                <div className="flex items-center gap-2 border-b-2 border-neutral-700 focus-within:border-primary transition-colors py-3">
              ...
              {currentField.fieldType === "PHONE" && (
                <div className="flex items-center gap-3 border-b-2 border-neutral-700 focus-within:border-primary transition-colors py-3 relative">
                  ...
                  {["YES", "NO"].map(opt => (
                    ...
                    >
                      <span className={`w-6 h-6 flex items-center justify-center font-extrabold text-xs border ${
                        answers[currentField.id] === opt ? "bg-primary text-black border-orange-400" : "border-neutral-600 text-neutral-400"
                      }`}>
                        {opt[0]}
                      </span>
                      {opt}
                    ...
                    return (
                      ...
                      >
                        <span className={`w-5 h-5 flex items-center justify-center shrink-0 border-2 transition-colors ${
                          checked ? "bg-primary border-orange-400" : "border-neutral-600"
                        }`}>
              ...
              {currentField.fieldType === "DROPDOWN" && (
                <div className="relative max-w-md mt-2">
                  <button
                    ...
                    className="w-full flex items-center justify-between border-2 border-neutral-700 hover:border-orange-400 px-4 py-3 text-sm font-black uppercase tracking-wider text-left transition-colors"
                  >
                    ...
                    <div className="absolute top-full left-0 mt-1 z-20 w-full bg-neutral-900 border border-neutral-700 shadow-2xl max-h-48 overflow-y-auto" onClick={e => e.stopPropagation()}>
                      {choices.map((opt, idx) => (
                        <button
                          ...
                          className="w-full text-left px-4 py-3 text-sm font-bold uppercase tracking-wide hover:bg-primary/10 hover:text-orange-400 transition-colors text-neutral-300"
                        >
                          {opt}
                        </button>
                      ))}
              ...
              {currentField.fieldType === "SLIDER" && (
                <div className="flex flex-col gap-4 max-w-lg mt-2">
                  <div className="relative w-full h-4 bg-neutral-800 border border-neutral-700">
                    <div className="absolute left-0 top-0 bottom-0 bg-primary" style={{ width: `${sliderPercent}%` }} />
                    <div
                      className="absolute top-1/2 -translate-y-1/2 w-6 h-6 bg-primary rotate-45 border-2 border-black"
                  ...
                  <div className="flex items-center justify-between text-xs font-black uppercase tracking-widest text-neutral-500">
                    <span>{min}</span>
                    <span className="text-orange-400 text-sm">{sliderValue}</span>
                    <span>{max}</span>
                  </div>
                </div>
              )}
                  ...
                  {childFields.map(child => (
                    <div key={child.id} className="flex flex-col gap-2">
                      ...
                      {child.fieldType === "EMAIL" ? (
                        ...
                        <input type="text" value={answers[child.id] ?? ""} onChange={e => setAnswer(child.id, e.target.value)} placeholder={child.placeholder || ""} className="bg-transparent border-b border-neutral-700 focus:border-primary text-white py-2 w-full focus-visible:outline-none transition-colors placeholder:text-neutral-600 text-base" />
                      )}
                    </div>
                  ))}
                </div>
                ...
                return (
                  <div className="flex flex-col gap-6 mt-2 max-w-lg w-full">
                    ...
                    <div className="flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-neutral-500 bg-neutral-900/60 p-3 border border-neutral-800">
                      <AlertCircle className="w-3.5 h-3.5 text-orange-500 shrink-0 animate-pulse" />
                    ...
                    {currentField.fieldType !== "AUDIO" ? (
                      <div className="flex flex-col gap-4">
                        <label className="border-2 border-dashed border-neutral-700 hover:border-orange-400 p-8 flex flex-col items-center justify-center gap-3 bg-neutral-900/30 transition-colors cursor-pointer text-center select-none group">
                          <input
                            type="file"
                            className="hidden"
                            accept={
                          ...
                          />
                          {currentField.fieldType === "IMAGE" && <ImageIcon className="w-8 h-8 text-neutral-500 group-hover:text-orange-400 transition-colors shrink-0" />}
                          {currentField.fieldType === "VIDEO" && <VideoIcon className="w-8 h-8 text-neutral-500 group-hover:text-orange-400 transition-colors shrink-0" />}
                          {currentField.fieldType === "FILE" && <FileText className="w-8 h-8 text-neutral-500 group-hover:text-orange-400 transition-colors shrink-0" />}
                      ...
                      <div className="flex flex-col gap-6 border-2 border-neutral-800 p-5 bg-neutral-900/10">
                        <h4 className="text-[10px] font-black uppercase tracking-widest text-orange-500 border-b border-neutral-800 pb-2">Voice Input & Upload Options</h4>
                        ...
                        <div className="flex flex-col gap-2">
                          ...
                          <label className="border border-neutral-700 hover:border-orange-400 p-3 flex items-center justify-center gap-2 cursor-pointer transition-colors bg-neutral-900/50">
                            <input
                              type="file"
                              className="hidden"
                              accept="audio/*"
                            ...
                            <div className="flex gap-2">
                              {!isRecording ? (
                                <button
                                  ...
                                  disabled={isUploading}
                                  className="w-full flex items-center justify-center gap-2 bg-primary hover:bg-primary/80 text-black font-black uppercase tracking-widest text-[10px] py-3 transition-colors"
                                >
                    ...
                    {currentUrl && (
                      <div className="border border-neutral-800 p-4 bg-neutral-900/50 flex flex-col gap-3">
                        <span className="text-[10px] font-black uppercase tracking-widest text-orange-400 flex items-center gap-1.5 border-b border-neutral-800 pb-2">
                          <Check className="w-3.5 h-3.5 text-orange-400 shrink-0" /> Successfully Uploaded
                        </span>
          ...
          {!["WELCOME", "INFO"].includes(currentField.fieldType) && (
            <div className="flex items-center gap-4 mt-4">
              ...
              <button
                ...
                className="flex items-center gap-2 bg-primary text-black font-black uppercase tracking-widest text-sm px-6 py-3 hover:bg-primary/80 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {isSubmitting ? (
